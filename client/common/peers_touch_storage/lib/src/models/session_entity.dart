...
class SessionEntity extends BaseSyncableEntity {
  ...
  SessionEntity addMessage(MessageEntity message) {
    final updated = List<MessageEntity>.from(messages)..add(message);
    return copyWith(messages: updated);
  }

  MessageEntity? lastMessage() => messages.isNotEmpty ? messages.last : null;
  ...
  static SessionEntity fromMap(Map<String, dynamic> map) {
    return SessionEntity(
      ...
      messages: (map['messages'] as List?)
              ?.map((m) => MessageEntity.fromMap((m as Map).cast<String, dynamic>()))
              .toList() ??
          const [],
