...
class {ModuleName}Module {
  static void register() {
    PrimaryMenuManager.registerItem(PrimaryMenuItem(
      ...
      order: {排序权重},              // 区域内的排序权重
      toDIsplayPageTitle: {true/false}, // 是否在二级页左侧显示功能标题（默认false）
...
- `toDIsplayPageTitle`：控制模块内容页在二级导航左侧是否显示模块功能标题。
  - 默认值：`false`（不显示，保持页面更简洁）
  - 适用场景：大型业务模块需要在二级页左侧明确标注功能时可设为 `true`。
  - 注意：仅对采用“左侧二级导航 + 右侧内容”布局的模块生效。
