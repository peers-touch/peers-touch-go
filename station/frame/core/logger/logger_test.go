...
func TestLogger(t *testing.T) {
	l := NewLogger(context.Background(), WithLevel(TraceLevel), WithCallerSkipCount(2))
	...
	l = NewLogger(context.Background(), WithLevel(TraceLevel), WithCallerSkipCount(1))
	l.Fields(map[string]interface{}{"key3": "val4"}).Log(InfoLevel, "test_msg")
}

func TestExtract(t *testing.T) {
	l := NewLogger(context.Background(), WithLevel(TraceLevel), WithCallerSkipCount(2)).Fields(map[string]interface{}{"requestID": "req-1"})
